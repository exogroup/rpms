...
%pre
%if 0%{?rhel} && "%rhel" < "9"
getent group 'squid' >/dev/null || groupadd -f -g '23' -r 'squid' || :
getent group 'squid' >/dev/null || groupadd -f -g '23' -r 'squid' || :
if ! getent passwd 'squid' >/dev/null; then
    if ! getent passwd '23' >/dev/null; then
        useradd -r -u '23' -g 'squid' -d '/var/spool/squid' -s '/sbin/nologin' -c 'Squid proxy user' 'squid' || :
    else
        useradd -r -g 'squid' -d '/var/spool/squid' -s '/sbin/nologin' -c 'Squid proxy user' 'squid' || :
    fi
fi
%else
